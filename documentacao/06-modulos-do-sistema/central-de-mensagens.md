...
O modulo usa o slug RBAC `atendimento_whatsapp`:

- `atendimento_whatsapp:visualizar` libera inbox, streams, metricas, chatbot, FAQ, respostas rapidas, fluxos, filas e configuracoes;
- `atendimento_whatsapp:editar` libera envio, sync manual, vinculo com OS, atualizacao de metadados e manutencao das automacoes da central.

Observacao de dominio:

- o endpoint `POST /atendimento-whatsapp/conversa/{id}/cadastrar-contato` continua exigindo tambem `clientes:criar` ou `clientes:editar`, porque a conversa pode criar ou atualizar cadastro oficial de contato/cliente no ERP.
...
## Hub mobile/PWA paralelo

A Central web continua sendo o nucleo operacional principal. O app mobile/PWA documentado em `documentacao/12-app-mobile-pwa/README.md` reutiliza o mesmo backend e banco, sem substituir a Central web existente.

Pontos confirmados no codigo:

- a entrada protegida no ERP web e `GET /atendimento-mobile`;
- o redirecionamento usa `mobile_pwa_url` quando configurado;
- o fallback padrao e `/atendimento-mobile-app/login`;
- o acesso de validacao em desktop continua disponivel via `?preview=1`;
- a versao documentada do app e `0.4.2`, com ERP minimo compativel `2.11.5`.
