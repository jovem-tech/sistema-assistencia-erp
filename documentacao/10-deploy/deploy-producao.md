...
- Data: `11/04/2026`
- Release: `v2.13.15`
- Metodo: patch seletivo via `tar.gz` + manifesto explicito de arquivos + migrations do modulo de orcamentos/pacotes + sincronizacao exclusiva das tabelas `pacotes_servicos` e `pacotes_servicos_niveis`
- Evidencia detalhada: `documentacao/10-deploy/2026-04-11-atualizacao-vps-release-v2.13.15.md`

## Resumo da implantacao atual

- O patch seletivo `v2.13.15` foi publicado em `/var/www/sistema-hml` sem enviar `.env`, `writable/`, `public/uploads/` ou dados ficticios gerais do ambiente local.
- As migrations novas do ERP foram aplicadas com sucesso, incluindo o modulo de pacotes de servicos e o fluxo de ofertas dinamicas de orcamento.
- Somente a configuracao operacional de pacotes de servico foi sincronizada para o banco de producao:
  - `pacotes_servicos`
  - `pacotes_servicos_niveis`
- Nenhum registro ficticio de clientes, OS, orcamentos, conversas, ofertas ou uploads locais foi enviado para a VPS nesta rodada.
- Validacoes operacionais apos a publicacao:
  - `https://sistema.jovemtech.eco.br/login` -> `200`
  - `https://sistema.jovemtech.eco.br/pacotes-servicos` -> `200`
  - `php8.3-fpm` -> `active`
  - `nginx` -> `active`
  - `pm2 describe whatsapp-gateway` -> `online`
- Artefatos operacionais gerados em `/root/deploy_patch_20260411_v2.13.15/`:
  - `sistema_hml_patch_20260411_v2.13.15.tar.gz`
  - `sync_manifest_20260411_v2.13.15.lf.txt`
  - `pacotes_servicos_data_20260411_v2.13.15.sql`
  - `postdeploy_files_20260411_v2.13.15.tar.gz`
