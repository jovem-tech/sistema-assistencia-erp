...
<div class="card glass-card">
    ...
    <input type="hidden" name="modelo_nome_ext" id="modelo_nome_ext">
            ...
            <!-- Navegação por Abas -->
            <ul class="nav nav-tabs nav-fill mb-4" id="equipamentoTabs" role="tablist">
            ...
            <div class="tab-content" id="equipamentoTabsContent">
                ...
                <div class="tab-pane fade" id="fotos-pane" role="tabpanel" aria-labelledby="fotos-tab" tabindex="0">
                    ...
                    <div id="fotoPreviewContainer" class="d-flex flex-wrap justify-content-center gap-4 mt-4">
                        <!-- Fotos Existentes -->
                        <?php if($isEdit && !empty($fotos)): ?>
                            ...
                            <div class="position-relative foto-item-wrapper" id="foto-existente-<?= $f['id'] ?>">
                                <div class="card shadow-sm border h-100" style="width: 160px;">
                                    <?php 
                                    $urlExistente = null;
                                    if (file_exists('uploads/equipamentos_perfil/' . $f['arquivo'])) {
                                        $urlExistente = base_url('uploads/equipamentos_perfil/' . $f['arquivo']);
                                    } elseif (file_exists('uploads/equipamentos/' . $f['arquivo'])) {
                                        $urlExistente = base_url('uploads/equipamentos/' . $f['arquivo']);
                                    }
                                    ?>
...
// (smartColorMap removido, usando PROFESSIONAL_COLORS)
    ...
    document.getElementById('btnConfirmCrop')?.addEventListener('click', () => {
        ...
        canvas.toBlob((blob) => {
            ...
            if (currentExisting + dt.items.length >= maxPhotos) {
                alert(`Você pode ter no máximo ${maxPhotos} fotos no total.`);
                modalCrop.hide();
                return;
            }

            const file = new File([blob], `equipamento_${Date.now()}.jpg`, { type: "image/jpeg" });
            dt.items.add(file);
            document.getElementById('fotoInputReal').files = dt.files;
            
            detectDominantColor(canvas); // <--- Identificação de Cor Adicionada
    ...
    // Preview ao escolher da galeria (Agora abre o cropper para cada foto)
    fotoInput?.addEventListener('change', function() {
        if (this.files && this.files[0]) {
            // Processa um por um para cropper (simplificado: pega a primeira do conjunto escolhido)
            const reader = new FileReader();
            reader.onload = e => {
                openCropper(e.target.result);
                fotoInput.value = ''; // Limpa para permitir re-seleção
            };
    ...
    // Adaptando deleção para o novo input real
    $(document).on('click', '.btn-del-foto-nova', function() {
        ...
        for(let i = 0; i < files.length; i++) {
            if(i !== index) dtNew.items.add(files[i]);
        }
    ...
    // Deletar foto existente via AJAX
    $(document).on('click', '.btn-del-foto-existente', function() {
