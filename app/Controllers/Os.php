...
class Os extends BaseController
{
    ...
    public function deleteEntryPhoto($fotoId)
    {
        $fotoId = (int) $fotoId;
        $fotoModel = new OsFotoModel();
        $foto = $fotoModel->find($fotoId);

        if (!$foto || strtolower(trim((string) ($foto['tipo'] ?? ''))) !== 'recepcao') {
            return $this->response
                ->setStatusCode(404)
                ->setJSON([
                    'success' => false,
        ...
        $arquivo = (string) ($foto['arquivo'] ?? '');
        $absolutePath = $this->resolveOsEntradaFotoAbsolutePath($arquivo);
        ...
        LogModel::registrar(
            'os_foto_entrada_excluida',
            'Foto de entrada removida da OS ID: ' . $osId . ' (foto ID ' . $fotoId . ').'
        );

        return $this->response->setJSON([
            'success' => true,
            'os_id' => $osId,
            'total_fotos' => $osId > 0 ? count($this->getOsEntradaFotosWithUrls($osId)) : 0,
            'fotos' => $osId > 0 ? $this->getOsEntradaFotosWithUrls($osId) : [],
            'csrfHash' => csrf_hash(),
        ]);
    }
    ...
    private function resolveOsGlobalSearchPlan(string $query, $db): array
    {
        ...
        if ($plan['cliente'] === null && $plan['cliente_telefone'] === null && $plan['equipamento'] === null && $plan['técnico'] === null && mb_strlen($query) >= 4) {
    ...
    private function buildOsWordPreview(string $text, int $wordsPerLine = 3, int $maxLines = 3, bool $attachPipeToPrevious = false): array
    {
        ...
        $lines = array_map(
            ...
            array_chunk($previewWords, max(1, $wordsPerLine))
        );

        $hasOverflow = count($parts) > count($previewWords);
        if ($hasOverflow && !empty($lines)) {
    ...
    private function resolveLongestVisibleWordForList(array $values): string
    {
        $longestWord = '-';
        $longestLength = function_exists('mb_strlen') ? mb_strlen($longestWord) : strlen($longestWord);
    ...
    private function formatOsRelatoCell(array $row): string
    {
        ...
        $preview = $this->buildOsRelatoPreview($relato, 3, 3);
        $linesHtml = implode('', array_map(
            static fn (string $line): string => '<span class="os-relato-line">' . esc($line) . '</span>',
            $preview['lines']
        ));
    ...
    private function buildOsRelatoPreview(string $relato, int $wordsPerLine = 3, int $maxLines = 3): array
    {
        $normalized = preg_replace('/\s+/u', ' ', trim($relato)) ?? trim($relato);
        if ($normalized === '') {
            return [
                'full' => '',
                'lines' => [],
        ...
        $visibleWordLimit = max(1, $wordsPerLine * $maxLines);
        $previewWords = array_slice($words, 0, $visibleWordLimit);
        $lines = array_map(
            static fn (array $chunk): string => implode(' ', $chunk),
            array_chunk($previewWords, max(1, $wordsPerLine))
        );
        ...
        return [
            'full' => $normalized,
            'lines' => $lines,
            'has_overflow' => count($words) > count($previewWords),
        ];
    }
    ...
    public function statusMeta($id)
    {
        ...
        $orcamentoViewData = array_merge(
            [
                'os' => $os,
                'orcamentoContext' => 'status_modal',
            ],
            $this->buildOsOrcamentoEditorViewData($os)
        );
        ...
        return $this->response->setJSON([
            ...
            'os' => [
                ...
                'procedimentos_executados' => (string) ($os['procedimentos_executados'] ?? ''),
                'solucao_aplicada' => (string) ($os['solucao_aplicada'] ?? ''),
    ...
    public function encerrarAjax($id)
    {
        ...
        if (! in_array($encerrarComo, $allowed, true)) {
            return $this->response->setStatusCode(422)->setJSON([
                'ok' => false,
                'message' => 'Selecione como a OS deve ser concluída.',
                'csrfHash' => csrf_hash(),
            ]);
        }
        ...
        foreach ($recebimentos as $index => &$recebimento) {
            ...
            if ($this->isCardPaymentMethod((string) ($recebimento['forma_pagamento'] ?? ''))) {
                if (! $cartaoService->isReady()) {
                    return $this->response->setStatusCode(422)->setJSON([
                        'ok' => false,
                        'message' => 'Configure primeiro as taxas de cartão no submenu Finanças > Cartões e taxas.',
                        'csrfHash' => csrf_hash(),
                    ]);
                }
                ...
                try {
                    ...
                    return $this->response->setStatusCode(422)->setJSON([
                        'ok' => false,
                        'message' => 'Lançamento #' . ($index + 1) . ': ' . $e->getMessage(),
                        'csrfHash' => csrf_hash(),
                    ]);
                }
        ...
        if ($totalRecebidoAgora > $saldoAntes + 0.001) {
            return $this->response->setStatusCode(422)->setJSON([
                'ok' => false,
                'message' => 'O total recebido nesta baixa não pode ultrapassar o saldo financeiro em aberto da OS.',
                'csrfHash' => csrf_hash(),
            ]);
        }
        ...
        if ($totalRecebidoAgora > 0 && ! $titulo) {
            return $this->response->setStatusCode(422)->setJSON([
                'ok' => false,
                'message' => 'Esta OS não possui valor financeiro em aberto para receber.',
                'csrfHash' => csrf_hash(),
            ]);
        }
        ...
        if ($deveProcessarBaixaOperacional && $comunicarCliente) {
            ...
            if (empty($notifyResult['ok'])) {
                $warningMessages[] = $notifyResult['message'] ?? 'A baixa foi registrada, mas o cliente não pôde ser comunicado.';
            }
        }
        ...
        if ($titulo) {
            ...
            if ($deveProcessarBaixaOperacional && $saldoAbertoAtual > 0.009) {
                ...
                if (trim((string) ($os['cliente_telefone'] ?? '')) === '') {
                    $warningMessages[] = 'A OS ficou com saldo pendente, mas o cliente não possui telefone válido para a cobrança automática por WhatsApp.';
                }
        ...
        if (! $deveProcessarBaixaOperacional) {
            $message = 'Pagamento antecipado registrado com sucesso. O status da OS não foi alterado.';
        } else {
            $message = $statusProcessado === 'entregue_pagamento_pendente'
                ? 'Baixa concluída. A OS ficou em aberto por saldo pendente e já entrou na régua automática de cobrança.'
    ...
    public function updateStatusAjax($id)
    {
        ...
        $detailUpdates = [];
        foreach (['procedimentos_executados', 'solucao_aplicada', 'diagnostico_tecnico'] as $field) {
            if ($this->request->getPost($field) !== null) {
                $detailUpdates[$field] = $this->normalizeNullableString($this->request->getPost($field));
            }
        }
    ...
    private function resolveAdministrativeApprovalForLockedOsChange(): array
    {
        if ($this->isCurrentUserAdministrator()) {
            return [true, 'Alterações financeiras autorizadas pelo administrador logado.'];
        }
        ...
        if ($adminUsuario === '' || $adminSenha === '') {
            return [false, 'Informe usuário e senha de um administrador para alterar valores após a baixa da OS.'];
        }

        $adminAprovador = $this->resolveAdministratorByIdentifier($adminUsuario);
        if (! $adminAprovador || ! password_verify($adminSenha, (string) ($adminAprovador['senha'] ?? ''))) {
            return [false, 'Credenciais de administrador inválidas.'];
        }

        return [true, 'Alterações financeiras autorizadas por administrador.'];
    }
    ...
    private function ensureOsFinancialMutationAllowedForCurrentUser(int $osId): ?string
    {
        ...
        return 'Esta OS já passou pela baixa. Apenas administradores podem alterar itens ou valores depois desse estágio.';
    }
    ...
    private function normalizeEncerramentoReceipts(string $rawJson): array
    {
        ...
        if (! is_array($decoded)) {
            throw new \InvalidArgumentException('Os recebimentos informados na baixa estão inválidos.');
        }
        ...
        foreach ($decoded as $row) {
            ...
            if (! in_array($formaPagamento, $allowedMethods, true)) {
                throw new \InvalidArgumentException('Existe um recebimento com forma de pagamento inválida.');
            }
    ...
    public function budgetSummary($id)
    {
        $os = $this->model->getComplete((int) $id);
        if (!$os) {
            return $this->response->setStatusCode(404)->setJSON([
        ...
        return $this->response->setJSON([
            'ok' => true,
            'html' => view('os/partials/orcamento_editor_panel', $viewData),
        ]);
    }
    ...
    public function edit($id)
    {
        ...
        $data = array_merge($data, $orcamentoViewData);
        return view('os/form', $data);
    }
    ...
    public function update($id)
    {
        ...
        try {
            if (array_key_exists('data_entrada', $dados)) {
                $dados['data_entrada'] = $this->normalizeNullableDateTimeInput((string) ($dados['data_entrada'] ?? ''));
            }
            if (array_key_exists('data_previsao', $dados)) {
                $dados['data_previsao'] = $this->normalizeNullableDateInput((string) ($dados['data_previsao'] ?? ''));
            }
        } catch (\InvalidArgumentException $e) {
            return redirect()->to($this->osEditUrl((int) $id))
                ->withInput()
        ...
        if (($dados['data_entrada'] ?? null) === null) {
            return redirect()->to($this->osEditUrl((int) $id))
                ->withInput()
                ->with('error', 'Informe a data de entrada.');
        }

        $entradaComparacao = $this->extractDateOnly((string) ($dados['data_entrada'] ?? ''));
        $previsaoComparacao = trim((string) ($dados['data_previsao'] ?? ''));
        ...
        $statusService = new OsStatusFlowService();
        $statusDisponiveis = array_values(array_filter(array_map(
            static fn (array $item): string => strtolower(trim((string) ($item['codigo'] ?? ''))),
            $statusService->buildTransitionHints(null)
        )));
        ...
        if ($this->isOsFinanciallyLocked($osAnterior) && $financialChanges !== []) {
            ...
            if (! $approved) {
                return redirect()->to($this->osEditUrl((int) $id))
                    ->withInput()
                    ->with('error', $adminApprovalMessage ?: 'Esta OS já passou pela baixa e exige autorização administrativa para alterar valores.');
            }
        }
        ...
        if ($statusAlterado) {
            ...
            if ($historicoModel->db->tableExists('os_status_historico')) {
                $historicoModel->insert([
                    'os_id' => (int) $id,
                    'status_anterior' => (string) ($osAnterior['status'] ?? ''),
        ...
        if ($financialChanges !== []) {
            LogModel::registrar(
                ...
                'Valores da OS ' . ($osAnterior['numero_os'] ?? ('#' . (int) $id))
                    . ' alterados após a baixa. Campos: ' . implode(', ', $financialChanges) . '.'
            );
        }
        ...
        $successMessage = $adminApprovalMessage !== null
            ? 'OS atualizada com autorização de administrador.'
    ...
    private function buildDefaultOsEmailSubject(array $os): string
    {
        ...
        $empresa = trim((string) get_config('empresa_nome', 'Assistência Técnica'));
    ...
    private function buildDefaultOrcamentoWhatsAppMessage(array $orcamento): string
    {
        ...
        $mensagem = 'Olá ' . $cliente . ', segue o orçamento ' . $numero . ' no valor total de ' . $total . '.';
        if ($tipoOrcamento === OrcamentoModel::TIPO_PREVIO) {
            ...
            $mensagem .= "\nEste documento considera o equipamento já recebido em assistência e a análise técnica realizada.";
        }
        if ($validade !== '-') {
            $mensagem .= "\nValidade: " . $validade . '.';
        }
    ...
    public function sendEmail($id)
    {
        $os = $this->model->getComplete((int) $id);
        if (!$os) {
            return redirect()->to('/os')->with('error', 'OS nï¿½o encontrada.');
        }
    ...
    public function generatePdf($id)
    {
        ...
        if ($tipo === '') {
            return redirect()->to($documentosTabUrl)->with('error', 'Tipo de documento não informado.');
        }
        $pdfOptions = [];
        if ($tipo === 'abertura') {
    ...
    private function persistEntryPhotosFromRequest(int $osId, string $numeroOs): ?string
    {
        if ($osId <= 0) {
            return null;
        }
        ...
        $files = [];
        foreach (['fotos_entrada', 'fotos_entrada[]'] as $field) {
        ...
        if (empty($files)) {
            foreach (['fotos_entrada', 'fotos_entrada[]'] as $field) {
                $single = $this->request->getFile($field);
                if ($single instanceof \CodeIgniter\HTTP\Files\UploadedFile && $single->isValid() && !$single->hasMoved()) {
                    $files[] = $single;
                    break;
                }
            }
        ...
        $targetDir = rtrim(FCPATH, '\\/') . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR . 'os_anormalidades';
        if (!is_dir($targetDir) && !@mkdir($targetDir, 0775, true) && !is_dir($targetDir)) {
            log_message('error', '[OS] diretorio de fotos de entrada indisponivel', [
                'os_id' => $osId,
        ...
        foreach ($files as $index => $img) {
            try {
                $ext = strtolower((string) $img->getExtension());
                if ($ext === '') {
                    $ext = 'jpg';
                }

                $newName = sprintf(
                    '%s_entrada_%d_%d_%04d.%s',
                    $slug,
                    $index + 1,
                    $timestamp,
                ...
                log_message('error', '[OS] falha ao persistir foto de entrada', [
                    'os_id' => $osId,
                    'file_name' => (string) ($img->getClientName() ?: ('entrada_' . ($index + 1))),
                    'target_dir' => $targetDir,
                    'message' => $e->getMessage(),
                ]);
            }
    ...
    private function buildOsEntradaFotoCandidates(string $arquivo): array
    {
        $arquivo = str_replace('\\', '/', ltrim($arquivo, '/'));
        if ($arquivo === '') {
            return [];
    ...
    {
        $candidates = $this->buildOsEntradaFotoCandidates($arquivo);
        if ($candidates === []) {
            return $this->missingImageDataUri();
        }
    ...
    private function resolveOsEntradaFotoAbsolutePath(string $arquivo): ?string
    {
        foreach ($this->buildOsEntradaFotoCandidates($arquivo) as $candidate) {
            if (is_file($candidate['path'])) {
