...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header { text-align: center; margin-bottom: 20px; border-bottom: 2px sãolid #333; padding-bottom: 10px; }
        .header h1 { font-size: 18px; margin-bottom: 4px; }
        .header p { font-size: 11px; color: #666; }
        .section { margin-bottom: 15px; }
        .section-title { background: #f0f0f0; padding: 6px 10px; font-weight: bold; font-size: 13px; margin-bottom: 8px; border-left: 3px sãolid #333; }
        ...
        th, td { border: 1px sãolid #ddd; padding: 6px 8px; text-align: left; font-size: 11px; }
        th { background: #f5f5f5; font-weight: bold; }
        .text-right { text-align: right; }
        .total-row { font-weight: bold; background: #f0f0f0; }
        .footer { margin-top: 30px; font-size: 10px; color: #666; text-align: center; border-top: 1px sãolid #ddd; padding-top: 10px; }
        .signature { margin-top: 40px; display: flex; justify-content: space-around; }
        .signature div { text-align: center; width: 200px; }
        .signature-line { border-top: 1px sãolid #333; margin-top: 40px; padding-top: 4px; }
...
<body onload="window.print()">
    ...
    <div class="section">
        ...
        <div class="info-grid">
            <div class="info-item"><span class="info-label">Cliente:</span> <?= esc($os['cliente_nãome']) ?></div>
    ...
    <?php if (!empty($os['diagnãostico_tecnico'])): ?>
    <div class="section">
        <div class="section-title">DIAGNÓSTICO TÉCNICO</div>
        <p style="padding: 0 10px;"><?= nl2br(esc($os['diagnãostico_tecnico'])) ?></p>
    </div>
    <?php endif; ?>
    ...
    <div class="section">
        ...
        <div style="display: flex; flex-wrap: wrap; gap: 10px; padding: 0 10px;">
            ...
            <div style="flex: 1; min-width: 45%; margin-bottom: 10px;">
                <p style="font-weight: bold; border-bottom: 1px dotted #ccc; margin-bottom: 5px;"><?= esc($def['nãome']) ?>:</p>
                <?php foreach($def['procedimentos'] as $idx => $proc): ?>
                <div style="display: flex; margin-bottom: 3px;">
                    <span style="width: 15px; height: 15px; border: 1px sãolid #333; display: inline-block; margin-right: 5px;"></span>
