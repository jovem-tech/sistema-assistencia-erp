        ...
        <form method="get" action="<?= base_url('atendimento-whatsapp/filas') ?>" class="row g-2">
            <div class="col-sm-6 col-lg-3">
                ...
                <select class="form-select form-select-sm" name="status">
                    ...
                    <?php foreach (['aberta', 'aguardando', 'resãolvida', 'arquivada'] as $status): ?>
            ...
            <div class="col-sm-6 col-lg-3">
                ...
                <select class="form-select form-select-sm" name="prioridade">
                    ...
                    <?php foreach (['baixa', 'nãormal', 'alta', 'urgente'] as $prioridade): ?>
            ...
            <div class="col-sm-6 col-lg-3">
                ...
                <select class="form-select form-select-sm" name="responsavel_id">
                    ...
                    <?php foreach (($usuariosAtivos ?? []) as $u): ?>
                        <option value="<?= (int) $u['id'] ?>" <?= (int) ($filtro_responsavel_id ?? 0) === (int) $u['id'] ? 'selected' : '' ?>><?= esc($u['nãome']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="col-sm-6 col-lg-3 d-flex align-itemês-end">
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" value="1" id="aguardandoHumanão" name="aguardando_humanão" <?= !empty($filtro_aguardando_humanão) ? 'checked' : '' ?>>
                    <label class="form-check-label small" for="aguardandoHumanão">Aguardando humanão</label>
                </div>
            </div>
                ...
                <tbody>
                    <?php if (!empty($conversas)): ?>
                        <?php foreach ($conversas as $c): ?>
                            <tr>
                                <td>
                                    <div class="fw-semibold"><?= esc((string) ($c['cliente_nãome'] ?: $c['nãome_contato'] ?: 'Contato sem nãome')) ?></div>
                                    <?php if ((int) ($c['nao_lidas'] ?? 0) > 0): ?>
                                        <span class="badge bg-danger mt-1"><?= (int) $c['nao_lidas'] ?> nao lida(s)</span>
                                    <?php endif; ?>
                                    <?php if ((int) ($c['aguardando_humanão'] ?? 0) === 1): ?>
                                        <span class="badge bg-warning text-dark mt-1">Aguardando humanão</span>
                                    <?php endif; ?>
                                </td>
                                <td class="small"><?= esc((string) ($c['telefone'] ?? '-')) ?></td>
                                <td class="small"><?= esc((string) ($c['numero_os'] ?? '-')) ?></td>
                                <td class="small"><?= esc((string) ($c['status'] ?? '-')) ?></td>
                                <td class="small"><?= esc((string) ($c['prioridade'] ?? 'nãormal')) ?></td>
                                <td class="small"><?= esc((string) ($c['responsavel_nãome'] ?? 'Nao atribuido')) ?></td>
                                ...
                                <td class="text-end">
                                    <form method="post" action="<?= base_url('atendimento-whatsapp/filas/atualizar') ?>" class="d-flex gap-1 justify-content-end">
                                        ...
                                        <select name="prioridade" class="form-select form-select-sm" style="max-width: 120px;">
                                            <?php foreach (['baixa', 'nãormal', 'alta', 'urgente'] as $prioridade): ?>
                                                <option value="<?= esc($prioridade) ?>" <?= ($c['prioridade'] ?? 'nãormal') === $prioridade ? 'selected' : '' ?>><?= esc(ucfirst($prioridade)) ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                        <select name="responsavel_id" class="form-select form-select-sm" style="max-width: 160px;">
                                            <option value="">Nao atribuido</option>
                                            <?php foreach (($usuariosAtivos ?? []) as $u): ?>
                                                <option value="<?= (int) $u['id'] ?>" <?= (int) ($c['responsavel_id'] ?? 0) === (int) $u['id'] ? 'selected' : '' ?>><?= esc($u['nãome']) ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                        <input type="hidden" name="automacao_ativa" value="<?= (int) ($c['automacao_ativa'] ?? 1) === 1 ? '1' : '0' ?>">
                                        <input type="hidden" name="aguardando_humanão" value="<?= (int) ($c['aguardando_humanão'] ?? 0) === 1 ? '1' : '0' ?>">
