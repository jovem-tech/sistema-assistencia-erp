...
class OrdersController extends BaseApiController
{
    public function index()
    {
        ...
        if ($q !== '') {
            ...
            $builder->groupStart()
                ...
                ->orLike('os.procedimentos_executados', $q)
    ...
    public function showByNumber($numeroOs = null)
    {
        if ($permissionError = $this->ensurePermission('os', 'visualizar')) {
            return $permissionError;
        }

        $numeroOs = urldecode(trim((string) $numeroOs));
        if ($numeroOs === '') {
            return $this->respondError('Numero da OS invalido.', 422, 'ORDER_INVALID_NUMBER');
        }

        $item = (new OsModel())->getCompleteByNumeroOs($numeroOs);
        if (!$item) {
            return $this->respondError('OS nao encontrada.', 404, 'ORDER_NOT_FOUND');
        }

        return $this->respondSuccess($this->buildWarrantyPayload($item));
    }

    public function showWarrantyPublic($numeroOs = null)
    {
        $numeroOs = urldecode(trim((string) $numeroOs));
        if ($numeroOs === '') {
            return $this->respondError('Numero da OS invalido.', 422, 'ORDER_INVALID_NUMBER');
        ...
        $signature = trim((string) ($this->request->getGet('sig') ?? ''));
        if (!$this->isWarrantyPublicSignatureValid($numeroOs, $signature)) {
            return $this->respondError('Link publico da garantia invalido ou expirado.', 403, 'WARRANTY_PUBLIC_SIGNATURE_INVALID');
        }
    ...
    public function create()
    {
        ...
        $data = [
            ...
            'procedimentos_executados' => $this->normalizeNullableString($payload['procedimentos_executados'] ?? null),
            'data_abertura' => $now,
    ...
    public function update($id = null)
    {
        ...
        if (array_key_exists('procedimentos_executados', $payload)) {
            $allowed['procedimentos_executados'] = $this->normalizeNullableString($payload['procedimentos_executados'] ?? null);
        }
     ...
     * @return array<string,mixed>
     */
    private function buildWarrantyPayload(array $item): array
    {
        $number = trim((string) ($item['numero_os'] ?? $item['numero_os_legado'] ?? ''));

        return [
            'id' => (int) ($item['id'] ?? 0),
            'number' => $number,
            'client' => trim((string) ($item['cliente_nome'] ?? '')),
            'equipment' => $this->buildWarrantyEquipmentSummary($item),
            'defect' => trim((string) ($item['relato_cliente'] ?? '')),
            'service' => $this->buildWarrantyServiceSummary($item),
            'warranty' => $this->buildWarrantyLabel($item),
            'technician' => trim((string) ($item['tecnico_nome'] ?? '')),
            'issued_at' => $this->formatWarrantyDate($item['data_abertura'] ?? $item['created_at'] ?? null),
            'notes' => $this->buildWarrantyNotes($item),
            'status' => trim((string) ($item['status'] ?? '')),
            'public_url' => $number !== '' ? $this->buildWarrantyPublicUrl($number) : null,
        ];
    }
     ...
     * @param array<string,mixed> $item
     */
    private function buildWarrantyEquipmentSummary(array $item): string
    {
        ...
        $serial = trim((string) ($item['equip_serie'] ?? ''));
        $imei = trim((string) ($item['equip_imei'] ?? ''));

        if ($serial !== '') {
            $summary .= ($summary !== '' ? ' | ' : '') . 'Serie: ' . $serial;
        }
        if ($imei !== '') {
            $summary .= ($summary !== '' ? ' | ' : '') . 'IMEI: ' . $imei;
        }

        return $summary !== '' ? $summary : 'Equipamento nao informado';
    }

    /**
     * @param array<string,mixed> $item
     */
    private function buildWarrantyServiceSummary(array $item): string
    {
        $parts = array_values(array_filter([
            trim((string) ($item['solucao_aplicada'] ?? '')),
            trim((string) ($item['procedimentos_executados'] ?? '')),
            trim((string) ($item['diagnostico_tecnico'] ?? '')),
        ]));

        $parts = array_values(array_unique($parts));
        return !empty($parts) ? implode(' | ', $parts) : 'Servico nao informado';
    }

    /**
     * @param array<string,mixed> $item
     */
    private function buildWarrantyLabel(array $item): string
    {
        $garantiaValidade = trim((string) ($item['garantia_validade'] ?? ''));
        $garantiaDias = (int) ($item['garantia_dias'] ?? 0);

        if ($garantiaValidade !== '') {
            $formatted = $this->formatWarrantyDate($garantiaValidade);
            return 'Valida ate ' . ($formatted !== '' ? $formatted : $garantiaValidade);
        }

        if ($garantiaDias > 0) {
            return $garantiaDias . ' dias';
        }
     ...
     * @param array<string,mixed> $item
     */
    private function buildWarrantyNotes(array $item): string
    {
        $notes = [];
        $cliente = trim((string) ($item['observacoes_cliente'] ?? ''));
        $interna = trim((string) ($item['observacoes_internas'] ?? ''));

        if ($cliente !== '') {
            $notes[] = 'Cliente: ' . $cliente;
        }
        if ($interna !== '') {
            $notes[] = 'Interna: ' . $interna;
        }
     ...
     * @param mixed $value
     */
    private function formatWarrantyDate($value): string
    {
        $raw = trim((string) $value);
        if ($raw === '') {
            return '';
    ...
    private function buildWarrantyPublicUrl(string $numeroOs): string
    {
        $numeroOs = trim($numeroOs);
        return site_url('api/public/warranty/' . rawurlencode($numeroOs)) . '?sig=' . rawurlencode($this->buildWarrantyPublicSignature($numeroOs));
    }
