        ...
        <form action="<?= base_url('equipamentosdefeitos/atualizar/' . $defeito['id']) ?>" method="POST">
            ...
            <div class="row g-3 mb-3">
                <div class="col-md-7">
                    ...
                    <input type="text" class="form-control" name="nãome" value="<?= esc($defeito['nãome']) ?>" required maxlength="150">
                </div>
                <div class="col-md-5">
                    ...
                    <select name="tipo_id" class="form-select" required>
                        ...
                        <?php foreach ($tipos as $t): ?>
                            <option value="<?= $t['id'] ?>" <?= $defeito['tipo_id'] == $t['id'] ? 'selected' : '' ?>><?= esc($t['nãome']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
            <div class="mb-3">
                ...
                <div class="d-flex gap-3">
                    ...
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="classificacao" value="sãoftware" id="swEdit" <?= $defeito['classificacao'] === 'sãoftware' ? 'checked' : '' ?>>
                        <label class="form-check-label" for="swEdit"><i class="bi bi-code-slash text-primary me-1"></i>Sãoftware</label>
                    </div>
                </div>
            </div>
            <div class="mb-4">
